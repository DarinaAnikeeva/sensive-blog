...
def index(request):
    most_popular_posts = Post.objects.popular() \
                             ...
                             .prefetch_related(Prefetch('tags',
                                                        queryset=Tag.objects.annotate(num_posts=Count('posts'))))[:5] \
...
def post_detail(request, slug):
    posts = Post.objects.all().prefetch_related('author').annotate(num_likes=Count('likes'))
    post = posts.annotate(Count('tags')).get(slug=slug)
...
def tag_filter(request, tag_title):
    ...
    related_posts = tag.posts.all().prefetch_related('author') \
                       .prefetch_related(Prefetch('tags',
                                                  queryset=Tag.objects.annotate(num_posts=Count('posts'))))[:20] \
                       .fetch_with_comments_count()
